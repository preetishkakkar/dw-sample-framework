...
VKAPI_ATTR VkBool32 VKAPI_CALL debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData)
{
    ...
    if (messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT || messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT)
        DW_LOG_INFO("Vulkan - " + message_type_str + " : " + std::string(pCallbackData->pMessage));
    if (messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
        DW_LOG_WARNING("Vulkan -" + message_type_str + " : " + std::string(pCallbackData->pMessage));
    if (messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
        DW_LOG_ERROR("Vulkan - " + message_type_str + " : " + std::string(pCallbackData->pMessage));
...
Image::Ptr Image::create_from_file(Backend::Ptr backend, std::string path, bool flip_vertical, bool srgb)
{
    ...
    if (ext == "hdr")
    {
        ...
        Image::Ptr image = std::shared_ptr<Image>(new Image(backend, VK_IMAGE_TYPE_2D, (uint32_t)x, (uint32_t)y, 1, 0, 1, VK_FORMAT_R32G32B32A32_SFLOAT, VMA_MEMORY_USAGE_GPU_ONLY, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_LAYOUT_UNDEFINED, x * y * sizeof(float) * 4, data));
    ...
    Object(backend), m_type(type), m_width(width), m_height(height), m_depth(depth), m_mip_levels(mip_levels), m_array_size(array_size), m_format(format), m_memory_usage(memory_usage), m_sample_count(sample_count), m_usage(usage), m_flags(flags), m_tiling(tiling)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create Image.");
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create Image View.");
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create Command Pool.");
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to allocate Command Buffer.");
...
{
    DW_ZERO_MEMORY(create_info);
    DW_ZERO_MEMORY(conservative_raster_create_info);
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create Compute Pipeline.");
...
AccelerationStructure::Desc::Desc()
{
    ...
    DW_ZERO_MEMORY(build_geometry_info);
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create sampler.");
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create Descriptor Set Layout.");
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create pipeline layout.");
    ...
    Object(backend)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create descriptor pool.");
...
void BatchUploader::build_blas(AccelerationStructure::Ptr acceleration_structure, const std::vector<VkAccelerationStructureGeometryKHR>& geometries, const std::vector<VkAccelerationStructureBuildRangeInfoKHR> build_ranges)
{
    ...
    {
        DW_LOG_FATAL("(Vulkan) Building a BLAS requires one or more Geometry and Build Offset structures.");
...
#    endif
    ...
    {
        DW_LOG_FATAL("(Vulkan) Failed to create Vulkan surface.");
    ...
    if (!find_physical_device(device_extensions, require_ray_tracing))
    {
...
std::vector<std::shared_ptr<ImageView>> Backend::swapchain_image_views()
{
...
std::shared_ptr<Image> Backend::swapchain_depth_image()
{
    return m_swap_chain_depth;
}
...
bool Backend::check_validation_layer_support(std::vector<const char*> layers)
{
    ...
    for (const char* layer_name : layers)
    {
        ...
        if (!layer_found)
        {
            DW_LOG_FATAL("(Vulkan) Validation Layer not available: " + std::string(layer_name));
            return false;
        }
    }
...
bool Backend::find_physical_device(std::vector<const char*> extensions, bool require_ray_tracing)
{
    ...
    for (const auto& device : devices)
    {
        ...
        if (is_device_suitable(device, VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU, infos, details, extensions, require_ray_tracing))
        {
    ...
    for (const auto& device : devices)
    {
        ...
        if (is_device_suitable(device, VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU, infos, details, extensions, require_ray_tracing))
        {
            m_vk_physical_device = device;
            m_selected_queues    = infos;
            m_swapchain_details  = details;
...
bool Backend::is_device_suitable(VkPhysicalDevice device, VkPhysicalDeviceType type, QueueInfos& infos, SwapChainSupportDetails& details, std::vector<const char*> extensions, bool require_ray_tracing)
{
...
bool Backend::find_queues(VkPhysicalDevice device, QueueInfos& infos)
{
    ...
    {
        DW_LOG_INFO("(Vulkan) No Presentation Queue Found");
        return false;
    }
    ...
    {
        DW_LOG_INFO("(Vulkan) No Graphics Queue Found");
        return false;
    }

    if (infos.compute_queue_quality == 0 || infos.transfer_queue_quality == 0)
    {
        DW_LOG_INFO("(Vulkan) No Queues supporting Compute or Transfer found");
        return false;
    }

    VkDeviceQueueCreateInfo presentation_queue_info;
    DW_ZERO_MEMORY(presentation_queue_info);
    ...
    DW_ZERO_MEMORY(graphics_queue_info);
    ...
    DW_ZERO_MEMORY(compute_queue_info);
    ...
    DW_ZERO_MEMORY(transfer_queue_info);
...
bool Backend::create_logical_device(std::vector<const char*> extensions, bool require_ray_tracing)
{
    ...
    DW_ZERO_MEMORY(device_ray_tracing_pipeline_features);
